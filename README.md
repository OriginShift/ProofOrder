...
The provider adapter checks the fixed task and snapshot schema, verifies that the order's input commitment matches its snapshot, derives the order digest from that same order, and evaluates one copied allocation value set before it seals the result or creates evidence. The allocation fixture shows how such rules could be used for a capped selection task, but its target metrics and choices are demo data. It is not a credible real purchasing example until the repository supplies real procurement inputs, documented eligibility rules, and an independently checkable basis for each metric.
