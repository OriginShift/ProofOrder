...
- Actual validation: 84 JavaScript tests and 7 contract tests passed; both CLI demos passed their assertions. The exchange counterexample is a security failure, retained in the evidence rather than marked as a satisfied guarantee. Full commands, environment and machine-readable outputs are in [decision 0011](../evidence/decisions/0011-encrypted-recovery-and-exchange-boundary.md).
