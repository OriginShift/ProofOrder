...
- 2026-09-13 address and verifier-input follow-up: Codex changed `readSettlementStatus()` to always use `contract.getAddress()` for the actual address, removed the unused block-number RPC call, and added the end-to-end regression where actual `0x0000000000000000000000000000000000000001` versus expected `0x0000000000000000000000000000000000000009` returns `CONTRACT_MISMATCH`. Codex removed the verifier CLI's plaintext result-file input branch; the lower-level unbound-plaintext refusal and its test remain. `node --test test/settlement-status.test.mjs` passed 12/12; Windows `npm ci --ignore-scripts` completed and reported zero vulnerabilities; a fresh WSL Linux-filesystem reproduction passed 198 JavaScript tests, 38 Foundry tests, the standalone 5-test CLI harness, and both local Anvil demos. WSL could not reach the Windows loopback-only audit proxy, so the reproduction wrapper checked the exact `package-lock.json` hash and propagated the real Windows npm audit result (0 vulnerabilities). Full log: [0019-codex-wsl-reproduce.log](../evidence/runs/0019-codex-wsl-reproduce.log). This is Codex-authored and Codex-run verification, not Chris's personal implementation, independent human review, or human acceptance; T20 remains pending.
