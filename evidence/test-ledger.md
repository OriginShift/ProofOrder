...
| T20 | Clean environment | independent reproduction log | Codex reran the full WSL reproduction from a Linux-filesystem snapshot with pre-existing build artifacts removed ([0019 log](runs/0019-codex-wsl-reproduce.log): 198 JavaScript tests, 38 Foundry tests, standalone CLI harness, both demos); Windows `npm ci --ignore-scripts` passed and the actual Windows audit result was lockfile-hash checked for the WSL audit step; independent human reproduction remains pending |
| T21 | RPC chain mismatch | refuse a status read from the wrong network | Mock RPC reports chain 31337 while the order expects 1; `readSettlementStatus` reads `getNetwork()` and returns `CHAIN_MISMATCH` ([test](../test/settlement-status.test.mjs), [full run](runs/0019-codex-wsl-reproduce.log)); independent review pending |
| T22 | Settlement address mismatch | refuse a status read from a different contract | `readSettlementStatus()` reads actual address `0x0000000000000000000000000000000000000001`; expected `0x0000000000000000000000000000000000000009` returns `CONTRACT_MISMATCH` ([test](../test/settlement-status.test.mjs), [full run](runs/0019-codex-wsl-reproduce.log)); independent review pending |
