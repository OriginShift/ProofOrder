...
1. The buyer's order binds the actual chain ID, deployed contract, parties, payee, verifier, random nonce, native payment amount, deadline, input snapshot/commitment and recipient public key. The on-chain evidence hash also includes `block.chainid`. Snapshot commitments use recursively sorted JSON and the `ProofOrder/InputSnapshot/v1` domain. The legacy digest helper retains its defaults, but this flow supplies those fields explicitly.
